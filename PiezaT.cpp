...
#include <ncurses.h>
using namespace std;
PiezaT::PiezaT(const Pieza& pp):Pieza(pp){
}
PiezaT::~PiezaT(){
//	delete [] tetrimino;
}
PiezaT::PiezaT() : Pieza(4,'T'){
...
void PiezaT::rotard(Pieza& p,int& nmov, int& nmovh, char** tab)const{
	//PiezaT pl= PiezaT();
	...
	if((nmov==3)||(nmovh==1)){//90 grados
		...
		tetrimino[1].setX(tetrimino[1].getX());
		tetrimino[1].setY(tetrimino[1].getY());
...
void PiezaT::rotari(Pieza& p,int& nmov, int& nmovh,char** tab)const{
	//PiezaT pl= PiezaT();
	Posicion* tetrimino= p.getTetrimino();	
	bool nodebe=false;
	if((nmov==1)||(nmovh==3)){//270 grados
...
void PiezaT::mover(Pieza& p,int nmov,char ** tab)const{
	Posicion* tetrimino= p.getTetrimino();	
	bool nodebe=false;
...
int PiezaT::menor(vector<int> pos){
	int men=20;
