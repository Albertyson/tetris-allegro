...
//#include <ncurses.h>
using namespace std;
PiezaJ::PiezaJ(const Pieza& pp):Pieza(pp){
}
PiezaJ::~PiezaJ(){
	//delete [] tetrimino;
}
PiezaJ::PiezaJ() : Pieza(4,'J'){
...
void PiezaJ::rotard(Pieza& p,int& nmov, int& nmovh, char** tab)const{
	//PiezaJ pl= PiezaJ();
...
void PiezaJ::rotari(Pieza& p,int& nmov, int& nmovh,char** tab)const{
	//PiezaJ pl= PiezaJ();
	Posicion* tetrimino= p.getTetrimino();	
	bool nodebe=false;
	if((nmov==1)||(nmovh==3)){//270 grados
...
void PiezaJ::mover(Pieza& p,int nmov,char ** tab)const{
	//allegro_message("Entro en abajo", allegro_error); 
	Posicion* tetrimino= p.getTetrimino();	
	bool nodebe=false;
...
int PiezaJ::menor(vector<int> pos){
	int men=20;
