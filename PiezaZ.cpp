...
#include <ncurses.h>
using namespace std;
PiezaZ::PiezaZ(const Pieza& pp):Pieza(pp){
}
PiezaZ::~PiezaZ(){
}
PiezaZ::PiezaZ() : Pieza(4,'Z'){
...
void PiezaZ::rotard(Pieza& p,int& nmov, int& nmovh, char** tab)const{
	//PiezaZ pl= PiezaZ();
...
void PiezaZ::rotari(Pieza& p,int& nmov, int& nmovh,char** tab)const{
	//PiezaZ pl= PiezaZ();
	Posicion* tetrimino= p.getTetrimino();	
	bool nodebe=false;
	if((nmov==1)||(nmovh==3)){//270 grados
...
void PiezaZ::mover(Pieza& p,int nmov,char ** tab)const{
	Posicion* tetrimino= p.getTetrimino();	
	bool nodebe=false;
...
int PiezaZ::menor(vector<int> pos){
	int men=20;
